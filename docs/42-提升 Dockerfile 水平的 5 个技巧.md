...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-1.png)

但是，这个容器和宿主机还是有关联的，比如可以把宿主机的端口映射到容器内的端口、宿主机某个目录挂载到容器内的目录。

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-2.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-3.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-4.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-5.png)

这时候访问http://localhost:3000可以看到 hello world，说明服务跑成功了:

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-6.png)
...
```
FROMnode:18WORKDIR/appCOPYpackage.json .COPY*.lock .RUNnpm config set registry https://registry.npmmirror.com/RUNnpm installCOPY. .RUNnpm run buildEXPOSE3000CMD["node","./dist/main.js"]
```
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-7.png)

然后在 docker desktop 的 images 里就可以看到这个镜像了：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-8.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-9.png)

然后就可以看到容器部分有了这个容器了：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-10.png)

浏览器访问http://localhost:2333就可以访问容器内跑的这个服务：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-11.png)

这就是 Dockerfile 构建成镜像，然后通过容器跑起来的流程。

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-12.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-13.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-14.png)

alpine 是高山植物，就是很少的资源就能存活的意思。

我们改下 dockerfile，使用 alpine 的镜像：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-15.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-16.png)

这次的 tag 为 second。

然后在 docker desktop 里看下：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-17.png)

好家伙，足足小了 900M。

我们点开看看：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-18.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-19.png)

docker desktop 可以看到这个跑起来的容器：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-20.png)

浏览器访问下，依然是正常的：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-21.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-22.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-23.png)

现在我们改下 README.md：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-24.png)

然后重新跑 build：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-25.png)

现在花了 25s，其实是没有重新 npm install 的。

然后改下 package.json：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-26.png)

再跑 docker build

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-27.png)

时间明显多了很多，过程中你可以看到在 npm install 那层停留了很长时间。

这就是为什么要这样写：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-28.png)
...
```
FROMnode:18-alpine3.14asbuild-stageWORKDIR/appCOPYpackage.json .RUNnpm config set registry https://registry.npmmirror.com/RUNnpm installCOPY. .RUNnpm run build# production stageFROMnode:18-alpine3.14asproduction-stageCOPY--from=build-stage /app/dist /appCOPY--from=build-stage /app/package.json /app/package.jsonWORKDIR/appRUNnpm config set registry https://registry.npmmirror.com/RUNnpm install --productionEXPOSE3000CMD["node","/app/main.js"]
```
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-29.png)

然后 desktop 里看下构建出来的镜像：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-30.png)

镜像体积比没有用多阶段构建的时候小了 250 M。

然后跑起来试试看：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-31.png)

这次映射 2335 端口到容器内的 3000 端口。

依然能正常访问：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-32.png)
...
```
export aaa=1 bbb=2node ./test.js
```

可以看到打印了这俩环境变量：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-33.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-34.png)

点击查看镜像详情，可以看到 ARG 已经被替换为具体的值了：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-35.png)

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-36.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-37.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-38.png)

其实还可以写成 ENTRYPOINT：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-39.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-40.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-41.png)

没有指定 --name 时，会生成一个随机容器名。

就是这种：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-42.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-43.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-44.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-45.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-46.png)

docker run:

```
docker run cmd-testdocker run cmd-test 66666
```

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-47.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-48.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-49.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-50.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-51.png)

可以看到，ADD 把 tar.gz 给解压然后复制到容器内了。

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-52.png)

而 COPY 没有解压，它把文件整个复制过去了：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-53.png)

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-54.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/42-55.png)
