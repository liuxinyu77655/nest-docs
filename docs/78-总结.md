...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-1.png)

javascript

```
setTimeout(()=>{thrownewError('xxx');},);
```

1s 以后抛一个错误，进程会终止。

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-2.png)
...
```
FROMnode:18-alpine3.14WORKDIR/appCOPY./index.js .CMD["node","/app/index.js"]
```
...
```
dockerbuild-trestart-test:first.
```

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-3.png)

在 docker desktop 里可以看到：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-4.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-5.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-6.png)

打印了很多次错误日志：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-7.png)

你可以点击停止，就不会再重启了：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-8.png)
...
```
FROMnode:18-alpine3.14WORKDIR/appCOPY./index.js .RUNnpm install -g pm2CMD["pm2-runtime","/app/index.js"]
```
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-9.png)

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-10.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-11.png)

这时候你会发现容器一直是运行状态，但是内部的进程一直在重启：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-12.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-13.png)

可以看到容器重启了 2 次，一共打印了 3 次错误就不再重启了：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-14.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-15.png)

可以看到容器一直在重启：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-16.png)

除非点击停止按钮，也就是执行 docker stop 才会停止：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-17.png)

```
docker stop restart-test-container5
```

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-18.png)

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-19.png)
...
![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-20.png)

这俩容器都停掉了。

现在我们重启 Docker：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-21.png)

他会重启跑 Docker Engine ，也就是 Docker Deamon 的后台进程。

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-22.png)

这时候你会发现，always 重启策略的容器又跑起来了，而 unless-stopped 的容器没有重启。这就是这俩的区别：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-23.png)

Docker Compose 是用于同时跑多个 Docker 容器的，它自然也支持 restart 的配置：

![](https://vercel-nestjs.oss-cn-beijing.aliyuncs.com/nest-docs/image/78-24.png)
