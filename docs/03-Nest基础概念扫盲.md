...
![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-1.png)

比如这里就是声明了一个 /user/create 的 post 接口，声明了一个 /user/list 的 get 接口。

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-2.png)
...
![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-3.png)

通过 @Query 来取 url 中的 query 参数，比如 /user/xx?id=222 里的 222

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-4.png)

通过 @Body 取 /book/create 的请求体内容：

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-5.png)

请求体一般会传递 json，比如`{ username: 'xxx', password: 'xxx' }`

我们会通过 dto （data transfer object）来接收。

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-6.png)

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-7.png)
...
![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-8.png)

**service 里做业务逻辑的具体实现，比如操作数据库等**

同理，/book/list、/book/create 接口是在另一个 BookController 里：

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-9.png)

它的业务逻辑实现也是在 BookService 里。

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-10.png)

很明显，UserController 和 UserService 是一块的，BookController 和 BookService 是一块的。

所以，Nest 有了模块的划分，每个模块里都包含 controller 和 service：

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-11.png)

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-12.png)
...
![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-13.png)
...
![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-14.png)

默认的 XxxService 只是一种简化的写法。

还可以直接 useValue 创建，通过 useFactory 创建等。

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-15.png)

刚才提到了 IoC 会自动从容器中查找实例来注入，注入的方式有两种：

属性注入和构造器注入。

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-16.png)
...
![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-17.png)
...
![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-18.png)
...
![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-19.png)

具体来说，有 Middleware、Guard、Interceptor、Pipe、Exception Filter 这五种。

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-20.png)

它们都是在目标 controller 的 handler 前后，额外加一段逻辑的。

比如你可以通过 interceptor 实现请求到响应的时间的记录：

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-21.png)

这种逻辑适合放在 controller 里么？

不适合，这种通用逻辑应该通过 interceptor 等方式抽离出来，然后需要用的时候在 controller 上声明一下：

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-22.png)
...
![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-23.png)

创建项目里的某个新模块：

![](https://liushuaiyang.oss-cn-shanghai.aliyuncs.com/nest-docs/image/3-24.png)
